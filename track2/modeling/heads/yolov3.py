...
from ppdet.modeling.post_process import JDEBBoxPostProcess

class YOLOv3(nn.Layer):
    """
    """
    def __init__(self, neck, yolo_head, post_process, for_mot=False):
        super().__init__()
        self.neck = neck
    ...
    def forward(self, body_feats, inputs):
        ...
        if self.training:
            yolo_losses = self.yolo_head(neck_feats, inputs)
            ...
            if self.for_mot:
                ...
                if self.return_idx:
                    ...
                    bbox, bbox_num = self.yolo_head.post_process(
                        yolo_head_outs, inputs['im_shape'],
                        inputs['scale_factor'])
