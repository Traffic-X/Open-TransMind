...
from modeling.layers import MultiHeadAttention, _convert_attention_mask
from modeling.transformers.position_encoding import PositionEmbedding
from modeling.initializer import linear_init_, conv_init_, xavier_uniform_, normal_
...
def build_transformer_detr_lazy(num_queries=100, position_embed_type='sine', nhead=8, num_encoder_layers=6,
    num_decoder_layers=6, dim_feedforward=2048, dropout=0.1, activation='relu', backbone_num_channels=2048, **kwargs):
    """
    Create a transformer instance from config
    """
    model = DETRTransformer(num_queries=num_queries, position_embed_type=position_embed_type, nhead=nhead, 
        num_encoder_layers=num_encoder_layers, num_decoder_layers=num_decoder_layers, dim_feedforward=dim_feedforward, 
        dropout=dropout, activation=activation, backbone_num_channels=backbone_num_channels)
