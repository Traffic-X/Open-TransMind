...
class SegmentationTransformer(nn.Layer):
    ...
    def __init__(self,
        ...
        self.proj_stride = head_config.get('proj_stride', 16)
    ...
    def forward(self, feats, inputs):
        if isinstance(feats, list):
            feats = feats[1] # all features
        x_shape = inputs['image'].shape
        logits = self.head(feats, self.cal_patch_x_shape(x_shape))
        output = [
            F.interpolate(
                _logit,
                x_shape[2:],
        ...
        if self.training:
            labels = inputs['label'].astype('int64')
            return self.loss(output, labels)
        else:
            return output

    def cal_patch_x_shape(self, x_shape):
        height, width = x_shape[2], x_shape[3]
        proj_height = height // self.proj_stride
        proj_width = width // self.proj_stride
        return [x_shape[0], 3, proj_height, proj_width]
